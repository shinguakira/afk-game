...
const INFRA_SPECS: InfraSpec[] = [
  ...
  },
  {
    ...
    icon: "kubernetes",
    services: [
      ["Pod / Deployment を作成", 1],
      ["Service / Ingress を設定", 4],
      ["ConfigMap / Secret を管理", 5],
      ["永続ボリューム", 7],
      ["Helm チャート作成", 8],
      ["HPA でオートスケール", 10],
      ["Operator を開発", 13],
  ...
  },
  {
    ...
    icon: "gcp",
    services: [
      ["Compute Engine で VM 構築", 1],
      ["Cloud Storage を構築", 2],
      ["Cloud IAM で権限設計", 3],
      ["VPC を構築", 4],
      ["Cloud SQL を運用", 5],
      ["Cloud Functions をデプロイ", 6],
      ["Cloud Run で配信", 7],
      ["BigQuery で分析", 8],
      ["Pub/Sub で連携", 9],
      ["GKE を運用", 11],
